CREATE OR REPLACE FUNCTION extension_budget_create_table () RETURNS TEXT AS $$
	CREATE TABLE extension.tbl_budgetantrag_status
	(
	...
	COMMENT ON TABLE extension.tbl_budgetantrag_status IS 'Statuses of Budget Requests';

	ALTER TABLE extension.tbl_budgetantrag_status ADD CONSTRAINT pk_tbl_budgetantrag_status PRIMARY KEY (budgetantrag_status_id);

	CREATE SEQUENCE extension.tbl_budgetantrag_status_budgetantrag_status_id_seq
	 INCREMENT BY 1
	 NO MAXVALUE
	 NO MINVALUE
	 CACHE 1;
	...
	GRANT SELECT, INSERT, UPDATE, DELETE ON extension.tbl_budgetantrag_status TO vilesci;
	GRANT SELECT, UPDATE ON extension.tbl_budgetantrag_status_budgetantrag_status_id_seq TO vilesci;
...
SELECT 
	CASE 
	WHEN (SELECT true::BOOLEAN FROM pg_catalog.pg_tables WHERE schemaname = 'extension' AND tablename = 'tbl_budgetantrag_status') 
